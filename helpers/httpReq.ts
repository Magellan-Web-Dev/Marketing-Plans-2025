...
export default class HttpReq {
    ...
    static async get(url: string = '', headers: Headers = {
      "Content-Type": ""
    }, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async post(url: string = '', data = null, headers: Headers = {
      "Content-Type": ""
    }, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async put(url: string = '', data = null, headers: Headers = {
      "Content-Type": ""
    }, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async #makeRequest(method: 'GET' | 'POST' | 'PUT' | 'DELETE' , url: string, data: null, headers: Headers = {
      "Content-Type": ""
    }, timeOut: number): Promise<ApiData> {
        ...
        if (!timeOut || Number(timeOut) < 1000) {
            return { ok: false, msg: "Timeout cannot be less than 1000ms or 1 second.", status: 0, data: null};
        }
