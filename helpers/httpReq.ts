...
export default class HttpReq {
    ...
    static async get(url: string = '', headers: object = {}, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async post(url: string = '', data = '', headers: object = {}, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async put(url: string = '', data = '', headers: object = {}, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async delete(url: string = '', headers: object = {}, timeOut: number = HttpReq.#defaultTimeout): Promise<ApiData> {
    ...
    static async #makeRequest(method: 'GET' | 'POST' | 'PUT' | 'DELETE' , url: string, data: any, headers: object, timeOut: number): Promise<ApiData> {
        ...
        if (!timeOut || Number(timeOut) < 1000) {
            return { ok: false, msg: "Timeout cannot be less than 1000ms or 1 second.", status: 0, data: []};
        }

        // Make sure header data is valid

        if (typeof headers !== 'object') {
            return { ok: false, msg: "Invalid header data.", status: 0, data: []};
        }

        // Check that url is not null

        if (!url) {
            return { ok: false, msg: "Invalid or missing URL.", status: 0, data: []};
        }
        ...
        const fetchData = async (): Promise<ApiData> => {
            ...
            try {
                ...
                if (!res.ok) {
                    console.error(`Server responded with a status code of '${res.status}' when attempting to fetch '${url}'`);
                    return { ok: false, msg: `Server responded with a status code of '${res.status}' when attempting to fetch '${url}'`, status: res.status, data: []};
                }
